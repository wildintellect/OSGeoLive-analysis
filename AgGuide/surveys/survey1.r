...
plotranks <- function(){
	...
	d1 <- dbGetQuery(con,'SELECT * FROM FeatureScore WHERE total > 0')
	...
	d2 <- dbGetQuery(con,'SELECT * FROM SearchScore WHERE total > 0')
	...
	barplot(d2$total, main="Search Rankings", horiz=TRUE, names.arg=c("Feature","Location","Seasonality","Product Group","Exact Product"), col=terrain.colors(5))
	dev.off()
}
